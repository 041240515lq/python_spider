...
def parser_to_excel(soup):
    global n
    # 查看网页可以看到我们要获取的信息都在class='interest-list'里面，所以获取到它，再获取到其中所有的li标签，组成一个list
    ...
    for book_item in content_list:
        ...
        # 获取图书的图片url
        book_pic_url = book_item.find_all('a')[0].find('img').get('src')
        # 获取图书的名称
        ...
        # 获取图书详情页面的信息并解析
        book_detail_content = get_url_content(book_detail_url)
        if book_detail_content == "404":
            continue
        else:
            book_detail_soup = BeautifulSoup(book_detail_content, 'html.parser')
            # parser_to_excel(book_detail_soup)
            #title = soup.find_all(class_='subject clearfix').text #.find_all('a')[0].get('title')
            ...
            # print("liu_book_len "+str(len(book_detail)))
            # print("liu_book_len " + repr(book_detail[1].find_next_sibling().name))
            # print(liu_book[0].next_element+" "+repr(liu_book[0].next_elements))#.next_sibling.next_element
            # print(liu_book[1].next_element+" "+liu_book[1].next_sibling)
            count_col = 1
            for span_item in book_detail:
                ...
                if span_item.find_next_sibling().name == "a":    #判断<span class="pl"> 的下一个元素是否是<a>标签
                    i = 0
...
if __name__ == "__main__":
    ...
    first_url = 'https://book.douban.com/people/154001100/wish'
    # 获取初始化页面的内容
